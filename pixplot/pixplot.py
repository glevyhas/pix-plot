...
def get_image_paths(**kwargs):
  ...
  if n_images <= min_clusters:
    print('\nError: min_clusters must be < input image count ')
    print('Found {} images and {} clusters were requested\n'.format(n_images, min_clusters))
    sys.exit()
...
def get_centroids(**kwargs):
  ...
  config = {
    'min_cluster_size': kwargs['min_clusters'],
...
def parse():
  ...
  parser.add_argument('--min_clusters', type=int, default=config['min_clusters'], help='the minimum number of clusters to identify', required=False)
